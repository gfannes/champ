...
- [*] When aggregating amps, retain max one amp per def to take the `org ~status/~date` over the parent
...
- [ ] Add support for `~prio` to fine-tune the order of the task list
	- Move datex into amp folder as well to handle all templated metadata in the same namespace
- [ ] Improve display of chores for search and plan
	- [ ] Collect all chores, sort, segment per different path and print these segments in reverse order
		- Support using normal order with `-r` or `-n`
	- [ ] Sort according to prio/match depending on the task
...
- [ ] Add CLI option `-d` for details to display amp metadata as well
