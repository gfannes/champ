...
impl App {
    ...
    pub fn run(&mut self) -> util::Result<()> {
        'mainloop: loop {
            self.term
                .process_events(self.config.mainloop_timeout_ms, |event| {
            ...
            let layout = tui::layout::Layout::create(&self.term)?;

            tui::text::Text::new(layout.path)
            ...
            tui::list::List::new(layout.location).draw(&mut self.term, &self.location_list)?;
            tui::list::List::new(layout.parent).draw(&mut self.term, &self.parent_list)?;
            tui::list::List::new(layout.preview).draw(&mut self.term, &self.preview_list)?;

            tui::text::Text::new(layout.status)
                .set_mark(is_filter_mode)
...
impl Config {
    pub fn load(cli_options: &config::CliArgs) -> util::Result<Config> {
        ...
        let global: config::Global = toml::from_str(content)?;
